...
## 1-1. 초안(데모) 배포 — DB 없이 화면 확인용

Vercel 환경변수에 `DEMO_MODE=1`, `ADMIN_PASSWORD`, `SESSION_SECRET`만 넣고 배포하면 DB·Blob 없이 동작합니다. 접수 내용은 서버 메모리에만 저장되어 **서버가 재시작되면 사라지고**, 화면 상단에 "초안(데모)" 안내가 표시됩니다. 실제 접수를 받기 전에는 반드시 `DEMO_MODE`를 지우고 Neon·Blob을 연결하세요.
